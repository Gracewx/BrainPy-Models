...
nb.profile.set(backend = "numba", dt = dt, merge_ing = True)
...
def LIF_model(Vr = 0., Vth = 20., Rm = 1., Cm = 10., tau_m = 10., refTime = 5., noise = 0.):
    ...
    g = lambda V, _t_, I_syn: noise / tau_m
    
    @nb.integrate (noise = g)
    def int_v(V, _t_, I_syn):  # integrate u(t)
        return (- ( V - Vr ) + Rm * I_syn) / tau_m
...
neu = nb.NeuGroup(LIF_neuron, geometry = (10, ), monitors = ['Vm'],
                  pars_update = {
...
plt.plot(ts, neu.mon.Vm[:, 0], label = f'neuron No.{0}, Vr = {neu._pars_to_update["Vr"][0]}mV, tau_m = {neu._pars_to_update["tau_m"][0]}ms.')
plt.plot(ts, neu.mon.Vm[:, 1], label = f'neuron No.{6}, Vr = {neu._pars_to_update["Vr"][6]}mV, tau_m = {neu._pars_to_update["tau_m"][6]}ms.')
