...
def get_ExpIF(V_rest=-65., V_reset=-68., V_th=-30., 
              V_T=-59.9, delta_T=3.48, a=1, b=.1,
              R=10., C=1., tau=10., tau_w = 10.,
    ...
    **Member name** **Initial Value** **Explanation**
    ...
    def int_V(V, _t_, w, I_ext):  # integrate u(t)
        return (- (V - V_rest) + delta_T * np.exp((V - V_T) / delta_T) -w + R * I_ext) / tau, noise / tau
    ...
    if mode == 'scalar':
        return bp.NeuType(name='adaExpIF_neuron',
                          requires=dict(ST=ST),
                          steps=(update, reset),
                          mode=mode)
    elif mode == 'vector':
